...
// SGL File
  ...
  public
    ...
    procedure Close;
    function DecodeFrame16(AFrame: Integer; ADst: PByte; AWidthBytes: Integer): Boolean;
    function DecodeFrame32(AFrame: Integer; ADst: PByte; AWidthBytes: Integer): Boolean;
    procedure DrawToCanvas( ImageIndex,FrameIndex:integer; canvas:TCanvas; x, y:integer );
...
function TSglFile.DecodeFrame16(AFrame: Integer; ADst: PByte;
  ...
  procedure Alpha5658(Alpha: Byte; ScrCol: Word; var DesCol: Word);
  var
    sR, sG, sB: Byte;
    dR, dG, dB: Byte;
    R, G, B: Byte;
        ...
        try
          ...
          try
            ...
            begin
              ...
              begin
                Alpha5658(lpACur^, lpSCur^, lpDCur^);
                Inc(lpACur);
                Inc(lpSCur);
                Inc(lpDCur);
              end;
...
begin
  ...
  try
    ...
    if FHeaderInfo.shOffset + FImageCount * (SizeOf(DWord) + 16) > FStream.Size then Exit;
...
procedure TSglFile.DrawToCanvas( ImageIndex,FrameIndex:integer; canvas:TCanvas; x, y:integer );
var
  bmTemp:TBitmap;
begin
  ...
  if FrameCount < 2 then
    ...
    FrameIndex := FrameIndex mod FrameCount;
  //bmTemp.Canvas.TextOut( x + 10, y + 10, '空' );
  if Frames[FrameIndex] <> nil then
  begin
    with Frames[FrameIndex]^ do
    begin
      bmTemp.PixelFormat := pf16Bit;
      bmTemp.Width := sfWidth;
      bmTemp.Height := sfHeight;
      if sfHeight > 0 then
      begin
        DecodeFrame16(FrameIndex, bmTemp.ScanLine[0], -((sfWidth * 2 + 3) and $FFFFFFFC));
        //Image1.Invalidate;
        canvas.Draw(x+sfX,y+sfY,bmTemp);
      end;    
    end;
  end;
  bmTemp.Free;
