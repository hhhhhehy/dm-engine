...
SERVER_ERROR CServer::AddEnterAccount(UINT nLoginId, const char* pszAccount, UINT& nSelCharId)
{
	...
	if (pExistId != nullptr)
	{
		LOGINENTER* pOld = m_EnterObject.Get(static_cast<UINT>(*pExistId));
		// 清理旧条目后再添加，防止槽位泄露
		UINT oldId = static_cast<UINT>(*pExistId);
		m_EnterObject.Del(oldId);
...
BOOL CServer::GetLoginEnterInfo(UINT nLoginId, UINT nSelCharId, LOGINENTER& info)
{
	...
	if (pInfo == nullptr)
	{
		// 清理孤儿映射：哈希表有记录但LOGINENTER已被释放
		m_Inthash.HDel(nLoginId);
		return FALSE;
	}
	if (pInfo->nSid != nSelCharId || pInfo->nLid != nLoginId)
	{
		// 验证失败，清理残留条目防止阻塞后续登录
		m_EnterObject.Del(id);
		m_Inthash.HDel(nLoginId);
